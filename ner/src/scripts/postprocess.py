...
def retrieve_offset_ann(refdir, preddir, output_ann_dir):    
    # create output dirs
    ...
    for cur_fn in glob(os.path.join(preddir, "**/*.ann"), recursive=True):

        offset_mapping = read_json(
            os.path.join(refdir, os.path.basename(cur_fn).replace(".ann", ".inv.map"))
        )
        reference = read_text(
        ...
        # write ann for brat
        write_lines(processed_lines, os.path.join(output_ann_dir, os.path.basename(cur_fn)))

        # write txt
        txt_fn = os.path.basename(cur_fn).replace(".ann", ".txt.ori")
...
if __name__ == '__main__':
    ...
    # ann files
        ...
        retrieve_offset_ann(sys.argv[1], sys.argv[2], sys.argv[3])

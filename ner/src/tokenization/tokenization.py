...
def parse_standoff_file(standoff_file, text_file, encoding=None):
    ...
    for line in read_lines(standoff_file, encoding=encoding):
        ...
        if line.startswith(
            ...
            cols = line.split("\t")
            entity_id, entity_annotation, entity_reference = cols[0], cols[1], cols[2]

            if len(cols) > 3: #for the case that the entity text has a tab character                
                entity_reference = '\t'.join(cols[2:])
                print(standoff_file + ': ' + line)
        ...
        elif line.startswith("M") : #or line.startswith("A"): --> close it for n2c2 2022 shared task
        ...
        elif line.startswith("A"): #this is for the n2c2 2022 Task 1 shared task
            attribute_id, attribute_value = line.split("\t", 1)
            attributes[attribute_id] = {
                "id": attribute_id.strip(),
                "value": attribute_value.strip(),
        ...
        for k in sorted(entities, key=lambda val: int(val.lstrip("TR"))):
            lines.append(
                    "{}\t{} {} {}\t{}".format(
                        ...
                        # entities[k]["type"],
                        "MedicalMention", #this is only for n2c2 2022
                        entities[k]["start"],
                        entities[k]["end"],
                        entities[k]["ref"],
                    )
                )
            # if entities[k]["type"] == 'Drug':
            #     lines.append(
            #         "{}\t{} {} {}\t{}".format(
            #             _normalise_triggers(entities[k]["id"]),
            #             # entities[k]["type"],
...
def build_subtoken_map(corpus_name, corpus_dir, output_dir):
    ...
    for fn in glob(os.path.join(corpus_dir, "**/*.txt"), recursive=True):
        ...
        a1_file = basename + ".ann" #Nhung updated this for other corpora converted from Pubtator format
...
def strip_invalid_chars(input_string):
    ...
    for fn in glob(os.path.join(corpus_dir, "**/*.txt"), recursive=True):
        ...
        while original_doc_pos < len(_original_doc) and normalized_doc_pos < len(
            ...
            if original_doc_char == normalized_doc_char:
                ...
                if original_doc_char == " " or original_doc_char == '�' \
                        or original_doc_char == '\uf062' or original_doc_char == '\uf02d' \
                        or original_doc_char == '\uf077' or original_doc_char == '\u0381'\
                        or original_doc_char == '\u200e' or original_doc_char =='\u200f':
                    offset_map[original_doc_pos] = normalized_doc_pos
                    original_doc_pos += 1
        ...
        for entity in entities.values():
            ...
            #Pará state: UTF_8 errors? 
            assert normalized_entity.strip()== normalized_entity and regex.sub(
                r"\s+", "", strip_invalid_chars(entity["ref"])
            ) == regex.sub(
                r" +", "", normalized_entity
            )  # Thank God
            # #Nhung added this to skip the case that the UFT-8 code was lost in sentence segmentation
            # if normalized_entity.strip() != normalized_entity or \
            #         regex.sub(r"\s+", "", entity["ref"]) != regex.sub(r" +", "", normalized_entity):
            #     print('Skip this entity: ', normalized_entity)
            entity_id = entity["id"]
...
def main():
    ...
    parser.add_argument('--indir', type=str, help='--indir', default='corpus/i2b2-2009')
    parser.add_argument('--outdir', type=str,help='--outdir', default='corpus/i2b2-2009-tok')

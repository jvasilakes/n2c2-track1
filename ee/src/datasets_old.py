...
class Collates():
    ...
    def _collate(self, data):
        ...
        bert_batch_seqs = pad_sequence(data[4], batch_first=True)
        bert_batch_mask = pad_sequence(data[5], batch_first=True)
        bert_batch_type = pad_sequence(data[6], batch_first=True)
        output = {'indxs':indx, 'labels':labels, 'names':names, 'input_ids':bert_batch_seqs,

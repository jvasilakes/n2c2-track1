...
def preprocess(data_path, folder_path, window):
    ...
    with open(join(folder_path,data_path.split('/')[-2]+'_data.txt'),"w") as fout:
        with open(join(folder_path,data_path.split('/')[-2]+'_special.txt'),"w") as fspecial:
        ...
        #         with open(join(folder_path+data_path.split('/')[-2],f+'.my'),"w") as fiout:

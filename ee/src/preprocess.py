...
def preprocess(data_path, preprocess_path, window):
    ...
    with open(join(preprocess_path,data_path.split('/')[-2]+'_data.txt'),"w") as fout:
        with open(join(preprocess_path,data_path.split('/')[-2]+'_special.txt'),"w") as fspecial:
            for f in tqdm(uniquefiles[0:]):
                ...
                with open(join(data_path,f+'.ann'), 'r') as infile:
        ...
        #         print(f)
                split_sentences = sentence_split_genia(orig_sentences)

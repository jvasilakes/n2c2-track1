...
def adjust_offsets(old_sents, new_sents, old_entities, f):
    ...
    while orgidx < orglen and newidx < newlen:
        ...
        if original[orgidx] == newtext[newidx]:
            ...
            newidx += 1
        elif original[orgidx] == '_': # biomedicus removes _
            orgidx += 1
        elif newtext[newidx] == '_':
            newidx += 1
        elif original[orgidx] == ':': # biomedicus removes : sometimes ex. 105-02
            orgidx += 1
        elif newtext[newidx] == ':':
            newidx += 1
        else:
...
#                 print('Change line here', newtext_break[term[0]:term[1]])
    ...
    for ts in terms.values():
        for term in ts:
            if newtext_break[term[0]:term[1]].replace(" ", "").replace("\n", "") != term[3].replace(" ", "").replace('\n', ''):
                if newtext_break[term[0]:term[1]].replace(" ", "").replace("\n", "").lower() == \
                        term[3].replace(" ", "").replace('\n', '').lower():
                    tqdm.write('ENT_ID {}, Lowercase Issue: {} <-> {}'.format(term[4], newtext_break[term[0]:term[1]], term[3]))
                    
                else:
                    tqdm.write('ENT_ID {}, Entities do not match: {} <-> {}'.format(term[4], newtext_break[term[0]:term[1]], term[3]))
                exit()
...
def preprocess_segmented(data_path, preprocess_path, segmented_path, window):
    ...
    ### Lets work with test 
    stats = {'entity_count':0, 'event_count': 0, 'event_type_count' :{'Disposition':0,'NoDisposition':0,'Undetermined':0},'unique_entity_count':0}
    with open(join(preprocess_path,data_path.split('/')[-2]+'_data.txt'),"w") as fout:
        with open(join(preprocess_path,data_path.split('/')[-2]+'_biomedicus_special.txt'),"w") as fspecial:
            for f in tqdm(uniquefiles[0:]):
                ...
                split_sentences = [sent_dict['_text'].replace('\n', ' ')  for sent_dict in sents_dict]

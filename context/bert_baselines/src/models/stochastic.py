...
from src.models.layers import TokenEmbeddingPooler, GumbelClassifier
from src.models.model_outputs import StochasticClassifierOutput
...
class BertMultiHeadedStochasticClassifier(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        self.stochastic_loss_fns = nn.ModuleDict()
        ...
        for (task, num_labels) in label_spec.items():
            self.classifier_heads[task] = nn.Sequential(
                    ...
                    GumbelClassifier(classifier_insize, 128, num_labels)
                    )
            ...
            stoch_loss_fn = get_loss_function("kl-div-categorical")
            self.stochastic_loss_fns[task] = stoch_loss_fn()
        ...
        for (task, clf_head) in self.classifier_heads.items():
            if dataset is not None:
                ...
                if dataset != task.split(':')[0]:
                    continue
            logits, alphas = clf_head(pooled_output)
            if labels is not None:
                task_labels = labels[task]
                clf_loss = self.classifier_loss_fns[task](logits, task_labels)
                stochastic_loss = self.stochastic_loss_fns[task](alphas)
            else:
                clf_loss = None
                stochastic_loss = None
            clf_outputs[task] = StochasticClassifierOutput(
                    loss=clf_loss,
    ...
    def training_step(self, batch, batch_idx):
        ...
        for (task, outputs) in outputs_by_task.items():
            total_loss += outputs.loss + outputs.stochastic_loss
            self.log(f"train_loss_{task}", outputs.loss)
            self.log(f"train_stochastic_loss_{task}", outputs.stochastic_loss)
        return total_loss

    def predict_from_logits(self, logits):
        if self.classifier_loss_fn == "binary-cross-entropy":
            # Multi-label classification
            softed = torch.sigmoid(logits)
            # TODO: 0.1 for testing. set back to 0.5
            preds = (softed > 0.1).float()
        else:
            softed = nn.functional.softmax(logits, dim=1)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for (task, outputs) in outputs_by_task.items():
            preds = self.predict_from_logits(outputs.logits)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        for (task, outputs) in outputs_by_task.items():
            ...
            task_metrics[task] = {"loss": outputs.loss,
                                  "stochastic_loss": outputs.stochastic_loss,
                                  "preds": preds,
    ...
    def validation_epoch_end(self, task_metrics):
        ...
        stochastic_losses_by_task = defaultdict(list)
        ...
        for example in task_metrics:
            for (task, metrics) in example.items():
                ...
                stochastic_losses_by_task[task].append(metrics["stochastic_loss"].detach().cpu().numpy())  # noqa
        ...
        for task in losses_by_task.keys():
            ...
            stochastic_losses_by_task[task] = np.array(
                stochastic_losses_by_task[task]).mean()
            ...
            self.log(f"val_stochastic_loss_{task}",
                     stochastic_losses_by_task[task], prog_bar=False)
